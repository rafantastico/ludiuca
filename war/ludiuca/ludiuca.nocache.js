...
(function($wnd, $doc){
  ...
  var badBrowser = 'Unable to load Super Dev Mode of "ludiuca" because\n';
  ...
  $wnd.__gwt_activeModules['ludiuca'] = {
    'moduleName' : 'ludiuca',
    'bindings' : function() {
      return {};
    }
 ...
 * "/ludiuca.nocache.js" is the one that loaded us. Normally this happens
...
function computeScriptBase() {
  ...
  $wnd.__gwt_activeModules['ludiuca'].superdevmode = true;

  var expectedSuffix = '/ludiuca.nocache.js';
  ...
  $wnd.alert('Unable to load Super Dev Mode version of ' + ludiuca + ".");
}
;
  ...
  var devModeHookKey = '__gwtDevModeHook:ludiuca';
  var devModeSessionKey = '__gwtDevModeSession:ludiuca';

  // Compute some codeserver urls so as the user does not need bookmarklets
  var hostName = $wnd.location.hostname;
  var serverUrl = 'http://' + hostName + ':9876';
  var nocacheUrl = serverUrl + '/ludiuca/ludiuca.nocache.js';
  ...
  $wnd.__gwt_activeModules['ludiuca'].canRedirect = true;
  $wnd.__gwt_activeModules['ludiuca'].superdevmode = true;
  ...
  $wnd[compileFunction] = function() {
    ...
    compileScript.src = serverUrl +
      '/recompile/ludiuca?user.agent=' + ua + '&_callback=' + callback;
